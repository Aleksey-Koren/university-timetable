package com.foxminded.koren.university.dao.jpaDao;
...
@Repository
public class JpaAudienceDao implements AudienceDao {

    private EntityManagerFactory entityManagerFactory;

    private static final Logger LOG = LoggerFactory.getLogger(JpaAudienceDao.class);

    public JpaAudienceDao() {

    }

    @Autowired
    public JpaAudienceDao(EntityManagerFactory entityManagerFactory) {
    ...
    public Audience getById(Integer id) {
        ...
        Audience audience = entityManager.find(Audience.class, id);
        if(audience == null) {
            throw new DAOException(String.format("There is no audience with id = %s in database", id));
        }
        entityManager.getTransaction().commit();
        entityManager.close();
        return audience;
